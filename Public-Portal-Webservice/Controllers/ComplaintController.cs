...
namespace Public_Portal_Webservice.Controllers
{
    public class ComplaintController : Controller
    {
        ...
        [NonAction]
        public vmPCP SevereComplaintsFind(vmPCP vm)
        {
            /////////////////////////start//////////////////////////////
        ...
        public ActionResult TownWiseAnnouncements(int? town_id,int?category,int? timePeriod )
        {
            ...
            try
            {
                ...
                if (town_id==0 &&category == null && timePeriod == null)
                {
                ...
                else if (category == null && timePeriod == null)
                {
                ...
                else
                {
                    ...
                    {
                        vm.annoucementsMultiple = db.Announcement1.ToList();
                        ViewBag.header = "All Town Announcements";
                        ViewBag.townId = 0;
                        vm.towns=db.Towns.ToList();
                    }
                    else
                    ...
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(a => a.category_id == category).ToList();
                        ViewBag.categoryId = category;
                    }
                    ...
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-3)) >= 0);
                        ViewBag.TimePeriodId = timePeriod;
                    }
                    else if (timePeriod == 6)
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-6)) >= 0); ViewBag.TimePeriodId = timePeriod;
                    }
                    else if (timePeriod == 9)
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-9)) >= 0); ViewBag.TimePeriodId = timePeriod;
                    }
                    else if (timePeriod == 12)
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-12)) >= 0); ViewBag.TimePeriodId = timePeriod;
                    }
                    else if (timePeriod == 24)
                    {
                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-24)) >= 0); ViewBag.TimePeriodId = timePeriod;

                    }
                    else if (timePeriod == 36)
                    {

                        vm.annoucementsMultiple = vm.annoucementsMultiple.Where(c => DateTime.Compare(c.date_creation, DateTime.Today.AddMonths(-36)) >= 0); ViewBag.TimePeriodId = timePeriod;
                    }
                }
        ...
        public ActionResult UCWiseAnnouncements(int? town_id,int? UC_id, int? category, int? timePeriod)
        {
            try
            {
                vmPCP vm = new vmPCP();
                ...
                ViewBag.UCId = UC_id;
                if (UC_id == 0 && category == null && timePeriod == null)
                {
                    vm.annoucementsMultiple = db.Announcement1.Where(uc => uc.UCArea.TownID == town_id).ToList();
                    ViewBag.header = "All "+ vm.town.town_name + " Announcements";
                    ViewBag.UCId = 0;
                ...
                {
                    vm.annoucementsMultiple = db.Announcement1.Where(a => a.UC_Area_id == UC_id).ToList();
                    vm.ucSingle = db.UCAreas.Find(UC_id);
                    ViewBag.header = "All " + vm.ucSingle.UCName + " UC Announcements";
                    //ViewBag.townId = vm.town.town_id;
                    vm.ucs = db.UCAreas.Where(a => a.TownID == town_id).ToList();
        ...
        [Authorize(Roles = "3")]
        public ActionResult ViewMyAnnouncements(int? category, int? timePeriod)
        {
            try
            {
                vmPCP vm = new vmPCP();
                ViewBag.pageMaporTownorAnnouncements = "MyAnnouncements";
                String username = System.Web.HttpContext.Current.User.Identity.Name;
                vm.acc = db.Accounts.Where(a => a.username.Equals(username)).FirstOrDefault();

                vm.annoucementsMultiple = db.Announcement1.Where(uc => uc.account_id==vm.acc.Account_id).ToList();
            
                vm.categories = db.Categories.ToList();

                if ( category == null && timePeriod == null)
                {
                    vm.annoucementsMultiple = db.Announcement1.ToList();
                    ViewBag.header = "All Announcements";
                    vm.towns = db.Towns.ToList();
        ...
        [Authorize(Roles = "3")]
        public ActionResult RegisterNewAnnouncement()
        {
            vmPCP vm = new vmPCP();
            ...
            String username = System.Web.HttpContext.Current.User.Identity.Name;
            vm.acc = db.Accounts.Where(a => a.username.Equals(username)).FirstOrDefault();
            vm.annoucementsMultiple = db.Announcement1.Where(a => a.UC_Area_id == vm.acc.UC_Area_id)                                 
                                .ToList();
        ...
        public ActionResult RegisterNewAnnouncement(vmPCP model)
        {
            //try
            // {
            ...
            using (PCP5Entities pcc = new PCP5Entities())
            {
                ...
                Announcement1 ccc = new Announcement1();

                ccc.title = model.compReg.c_title;
                ...
                ccc.image_file_1 = "~/Image/" + fileName;
                ...
                if (model.compReg.imageFile2 != null)
                {
                    ...
                    fileName = fileName + DateTime.Now.ToString("yymmssfff") + extension;
                    ccc.image_file_2 = "~/Image/" + fileName;
                    fileName = Path.Combine(Server.MapPath("~/Image/"), fileName);
                ...
                {
                    fileName = "NoImageFound.jpg193925393.png";
                    ccc.image_file_2 = "~/Image/" + fileName;
                ...
                if (model.compReg.imageFile3 != null)
                {
                    ...
                    extension = Path.GetExtension(model.compReg.imageFile2.FileName);
                    fileName = fileName + DateTime.Now.ToString("yymmssfff") + extension;
                    ccc.image_file_3 = "~/Image/" + fileName;
                    fileName = Path.Combine(Server.MapPath("~/Image/"), fileName);
                    model.compReg.imageFile2.SaveAs(fileName);
                }
                else
                ...
                pcc.Announcement1.Add(ccc);
                pcc.SaveChanges();
            }

            return RedirectToAction("ViewMyAnnouncements");
        ...
        public ActionResult AddFeedback(int complaintId,String FeedbackMssg, int rating, String urlprev)
        {
            String compIdd = complaintId.ToString();
            Feedback fdb = new Feedback();
            fdb.FeedbackMessage = FeedbackMssg;
            fdb.complaint_id = complaintId;
            fdb.Rating = rating;
            fdb.time_created = DateTime.Now;

            db.Feedbacks.Add(fdb);
            db.SaveChanges();


            return RedirectToAction("SingleComplaint",new { Complaint_ID = compIdd, url=urlprev });
        }
        public ActionResult SingleComplaint(String Complaint_ID,String url)
        {
            try
            {
                ...
                ViewBag.CheckFeedback = "null";
                if (Request.IsAuthenticated && User.IsInRole("5"))
                {
                    if (vm.com.account_id == vm.acc.Account_id)
                    {
                        if (vm.com.stage_id == 9)
                        {
                            var checking = db.Feedbacks.Where(a => a.complaint_id == comp_id_int).FirstOrDefault();
                            if (checking == null)
                            {
                                ViewBag.CheckFeedback = "Y";
                            }
                        }
                    }
                }
